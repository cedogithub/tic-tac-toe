...
const playRound = (index) => {
  ...
  if (isCellUpdated) {
    ...
    if (isWinner) {
      ...
      // Add the "show" class to trigger the fade animation
      winnerElement.classList.add("animate");

      // Set a timeout to remove the "show" class and reset the opacity after the animation
      setTimeout(() => {
        winnerElement.classList.remove("animate");
      }, 3500);

      // // Reveal the winning crown image for the current player
      // const currentPlayerIndex = currentPlayer === player1 ? 1 : 2;
      // const crownImage = document.getElementById(`crown${currentPlayerIndex}`);
